...
const BookingItem = ({ reservation }: Props) => {
  return (
    <div className="border border-slate-300 rounded-lg border-solid p-4">
      ...
      <Item
        title="Date:"
        value={dayjs(reservation.bookingDate).format("DD/MM/YYYY")}
      />
      <Item
        title="Start time:"
        value={dayjs(reservation.startTime).format("HH:mm")}
      />
      <Item
        title="End time:"
        value={dayjs(reservation.endTime).format("HH:mm")}
      />
      <Item
        title="Total:"
        value={CurrencyHelper.formatVND(reservation.total)}
      />
    </div>
  );
};
